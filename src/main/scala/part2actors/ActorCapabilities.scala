...
import part2actors.ActorCapabilities.Person.obj
...
object ActorCapabilities extends App {
  ...
  class SimpleActor extends Actor {
    override def receive: Receive = {
      ...
      case SpecialMessage(contents) => println(s"[simple actor] I have received something: $contents")
  ...
  //   messages can be of any type & must be Immutable and Serializable
  ...
  // D -> A -> B
  // forwarding = sending a message with the orig. sender
   ...
   * Exercise :
   *
   * 1. a Counter actor which takes below params
   *   - Increment
   *   - Decrement
   *   - Print
   *
   * 2. a Bank account as an actor which takes below params
   *   receives
   *   - Deposit an amount
   *   - Withdraw an amount
   *   - Statement
...
"""
  ...
  object Person {
    case class obj(account: ActorRef)
  }
